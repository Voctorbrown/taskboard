...
function handleCardLongPress(cardElement) {
    ...
    const columnId = cardElement.parentElement.dataset.columnId;
    
    currentLongPressCard = {
        cardId: cardId,
        columnId: columnId
    };
...
function showMoveCardModal() {
    ...
    const boardList = document.getElementById('move-card-board-list');
    
    if (!modal || !boardList) return;
    ...
    data.boards.forEach(board => {
        ...
        countSpan.textContent = `${stats.done}/${stats.total}`;
        
        boardItem.appendChild(nameSpan);
        boardItem.appendChild(countSpan);
        
        boardItem.onclick = function() {
            moveCardToBoard(currentLongPressCard.cardId, board.id);
            closeMoveCardModal();
        };
    ...
    if (data.boards.length <= 1) {
        ...
        emptyMsg.textContent = '没有其他看板可移动';
        emptyMsg.style.textAlign = 'center';
        emptyMsg.style.color = '#666';
        boardList.appendChild(emptyMsg);
    }
...
function moveCardToBoard(cardId, targetBoardId) {
    if (!currentLongPressCard) return;
    
    const sourceBoard = getCurrentBoard();
    ...
    for (let col of sourceBoard.columns) {
    ...
    // 从原看板移除卡片
    const cardIndex = sourceColumn.cards.indexOf(card);
    if (cardIndex > -1) {
    ...
    if (targetBoard.columns.length === 0) {
        targetBoard.columns.push({
            id: generateId(),
            title: '待办',
            cards: [card]
        });
    } else {
        targetBoard.columns[0].cards.push(card);
    }
...
function setupMobileEvents() {
    ...
    // 移动卡片弹窗关闭按钮
    if (moveCardModal) {
        const closeBtn = moveCardModal.querySelector('.close');
        if (closeBtn) {
            closeBtn.addEventListener('click', closeMoveCardModal);
        }
        
        // 点击外部关闭
        moveCardModal.addEventListener('click', function(e) {
...
function setupCardLongPress() {
    ...
    cards.forEach(card => {
        card.addEventListener('touchstart', function(e) {
            ...
            cardLongPressTimer = setTimeout(function() {
                handleCardLongPress(target);
            }, 500);
        }, { passive: true });
