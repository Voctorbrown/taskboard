...
const DEFAULT_COLUMNS = [
    { id: 'todo', name: '🎯待办', isDone: false, isPinned: true },
    { id: 'template', name: '🧩模板', isDone: false, isPinned: true },
    { id: 'emergence', name: '🚨紧急', isDone: false, isPinned: true },
    { id: 'done', name: '✅已完成', isDone: true, isPinned: true }
];
...
function validateAndMigrateBoard(board) {
    ...
    if (board.columnConfig && Array.isArray(board.columnConfig)) {
        board.columnConfig.forEach(function(oldCol) {
            ...
            // 把旧的inprogress id映射到emergence
            if (oldCol.id === 'inprogress') {
                targetId = 'emergence';
            }
                ...
                Object.keys(board.columns).forEach(function(key) {
                    if (Array.isArray(board.columns[key])) {
                        ...
                        if (key === 'paused') {
                            ...
                            // 把inprogress板块迁移到emergence
                            migrated.columns['emergence'] = board.columns[key];
                        } else {
...
function createDefaultData() {
    ...
    annualGoalBoard.columns['todo'] = [
        { id: generateId(), content: '完成年度财务目标', color: '#ffcccc', createdAt: getTodayString(), pausedAt: null, completedAt: null },
        { id: generateId(), content: '读完12本书', color: '#fff3cd', createdAt: getTodayString(), pausedAt: null, completedAt: null }
    ];
    ...
    urgentBoard.columns['todo'] = [
        { id: generateId(), content: '处理紧急工作邮件', color: '#ffcdd2', createdAt: getTodayString(), pausedAt: null, completedAt: null }
    ];
...
function renderWeekView() {
    ...
    let html = '<div class="stats-summary">' +
        ...
        '<div class="stat-card emergence"><h4>紧急</h4><div class="stat-number ' + (stats.totalEmergence === 0 ? 'value-zero' : 'value-non-zero') + '">' + stats.totalEmergence + '</div></div>' +
...
function getWeekStats() {
    ...
    currentBoard.columnConfig.forEach(function(colConfig) {
        ...
        else if (colConfig.id === 'emergence') totalEmergence = count;
...
function showBadgeModalSequence(badges, index) {
    ...
    badgeIconContainer.innerHTML = `<img src="images/${milestone}.png" alt="${milestone}徽章">`;
    badgeTitle.innerHTML = `🎉 恭喜获得「${BADGE_NAMES[milestone]}」徽章！`;
    badgeText.innerHTML = BADGE_ENCOURAGEMENT[milestone];
...
function renderBadges() {
    ...
    sortedBadges.forEach(milestone => {
        ...
        badgeImg.src = `images/${milestone}.png`;
        badgeImg.alt = `${BADGE_NAMES[milestone]}`;
        badgeImg.title = `「${BADGE_NAMES[milestone]}」- 完成 ${milestone} 个任务 - 点击查看徽章介绍`;
...
function showBadgeInfoModal() {
    ...
    statsTitle.innerHTML = '📊 我的统计';
    
    const totalCardsDiv = document.createElement('div');
    totalCardsDiv.className = 'badge-stat-item';
    totalCardsDiv.innerHTML = `<span class="stat-label">累计创建卡片：</span><span class="stat-value">${totalCards}</span>`;
    
    const totalCompletedDiv = document.createElement('div');
    totalCompletedDiv.className = 'badge-stat-item';
    totalCompletedDiv.innerHTML = `<span class="stat-label">累计完成卡片：</span><span class="stat-value">${totalCompleted}</span>`;
    ...
    BADGE_MILESTONES.forEach(milestone => {
        ...
        iconImg.src = `images/${milestone}.png`;
        ...
        titleH3.innerHTML = BADGE_NAMES[milestone];
        
        const statusSpan = document.createElement('span');
        statusSpan.className = `badge-info-status ${isEarned ? 'earned' : 'not-earned'}`;
        statusSpan.innerHTML = isEarned ? '✅ 已获得' : '🔒 未获得';
        ...
        requirementP.innerHTML = `获得条件：完成 ${milestone} 个任务`;
        
        const encouragementP = document.createElement('p');
        encouragementP.className = 'badge-info-encouragement';
        encouragementP.innerHTML = BADGE_ENCOURAGEMENT[milestone];
