...
const SORT_STATES_STORAGE_KEY = 'dodo-kanban-sort-states';
...
function loadFromStorage() {
    ...
    // 加载排序状态（需要在data.currentBoardId设置后）
    loadSortStatesFromStorage();
            ...
            if (typeof board.columns === 'object') {
                Object.keys(board.columns).forEach(function(key) {
                    if (Array.isArray(board.columns[key])) {
                        ...
                        // 迁移卡片数据：去除pausedAt字段，并更新时间格式
                        const cards = board.columns[key].map(card => {
                            const { pausedAt, ...rest } = card;
                            
                            // 确保时间格式是完整的（YYYY-MM-DD HH:MM:SS）
                            if (rest.createdAt && !rest.createdAt.includes(' ')) {
                                rest.createdAt = rest.createdAt + ' 00:00:00';
                            }
                            if (rest.completedAt && !rest.completedAt.includes(' ')) {
                                rest.completedAt = rest.completedAt + ' 23:59:59';
                            }
...
function initFriendLinksFavicons() {
    const container = document.getElementById('friend-links-container');
    if (!container) return;
    
    const links = container.querySelectorAll('.friend-link');
    
    links.forEach((link, index) => {
        const faviconIndex = index + 1;
        ...
        // 先尝试png格式
        img.src = `images/aboutUs-favicon${faviconIndex}.png`;
        
        // 如果png加载失败，尝试ico格式
        img.onerror = function() {
            // 如果当前不是ico，再尝试ico
            if (!this.src.includes('.ico')) {
                this.src = `images/aboutUs-favicon${faviconIndex}.ico`;
                // 如果ico也失败，隐藏图标
...
function saveSortStatesToStorage() {
    // 加载已保存的排序数据
    let allSortStates = {};
    const stored = localStorage.getItem(SORT_STATES_STORAGE_KEY);
    if (stored) {
        try {
            allSortStates = JSON.parse(stored);
        } catch (e) {
            console.error('解析排序状态失败:', e);
        }
    }
    
    // 保存当前看板的排序状态
    if (data.currentBoardId) {
        allSortStates[data.currentBoardId] = columnSortStates;
    }
    
    localStorage.setItem(SORT_STATES_STORAGE_KEY, JSON.stringify(allSortStates));
    console.log('已保存排序状态:', columnSortStates);
}

function loadSortStatesFromStorage() {
    const stored = localStorage.getItem(SORT_STATES_STORAGE_KEY);
    if (stored) {
        try {
            const allSortStates = JSON.parse(stored);
            // 加载当前看板的排序状态
            if (data.currentBoardId && allSortStates[data.currentBoardId]) {
                columnSortStates = allSortStates[data.currentBoardId];
                console.log('已加载排序状态:', columnSortStates);
            }
        } catch (e) {
            console.error('解析排序状态失败:', e);
        }
...
function fixAllCardTimeFormats() {
    console.log('检查并修复所有卡片的时间格式...');
    let fixedCount = 0;
    
    const allBoardLists = [data.boards, data.completedBoards, data.discardedBoards];
    
    allBoardLists.forEach(function(boardList) {
        boardList.forEach(function(board) {
            if (board && board.columns) {
                Object.keys(board.columns).forEach(function(colId) {
                    const cards = board.columns[colId];
                    if (Array.isArray(cards)) {
                        cards.forEach(function(card) {
                            // 修复 createdAt
                            if (card.createdAt && !card.createdAt.includes(' ')) {
                                card.createdAt = card.createdAt + ' 00:00:00';
                                fixedCount++;
                            }
                            // 修复 completedAt
                            if (card.completedAt && !card.completedAt.includes(' ')) {
                                card.completedAt = card.completedAt + ' 23:59:59';
                                fixedCount++;
                            }
                        });
                    }
    ...
    if (fixedCount > 0) {
        console.log('修复了 ' + fixedCount + ' 个时间格式');
        saveToStorage();
    }
}
...
function createColumnElement(colConfig, board) {
    ...
    pinBtn.title = colConfig.isPinned ? '编辑模式（拖动卡片为移动，删除原卡片）' : '模板模式（拖动卡片为复制，不删除原卡片）';
...
function createCardElement(card, columnId) {
    ...
    if (columnId === '2') {
        if (card.createdAt) {
            dateInfo += '<div class="card-date">创建：' + formatDateOnly(card.createdAt) + '</div>';
        }
    } else if (columnId === '4') {
        dateInfo += '<div class="card-dates-row">';
        if (card.createdAt) {
            dateInfo += '<div class="card-date card-date-left">创建：' + formatDateOnly(card.createdAt) + '</div>';
        }
        if (card.completedAt) {
            dateInfo += '<div class="card-date card-date-right">完成：' + formatDateOnly(card.completedAt) + '</div>';
        }
        dateInfo += '</div>';
    } else {
        if (card.createdAt) {
        ...
        if (card.completedAt) {
            dateInfo += '<div class="card-date">完成：' + formatDateOnly(card.completedAt) + '</div>';
        }
    }
    ...
    if (cardToMove && sourceCards) {
        ...
        if (sourceColumnConfig && !sourceColumnConfig.isPinned) {
            ...
            if (targetColumnConfig && targetColumnConfig.isDone) {
                newCard.completedAt = getNowString();
            }
            ...
            if (sourceColumnId !== targetColumnId) {
                if (targetColumnConfig && targetColumnConfig.isDone) {
                    ...
                    if (!cardToMove.completedAt) {
                        cardToMove.completedAt = getNowString();
                    }
                } else {
        ...
        if (targetColumnId) {
            const targetCards = currentBoard.columns[targetColumnId] || [];
            const sortedCards = sortCards(targetCards, targetColumnId);
            currentBoard.columns[targetColumnId] = sortedCards;
        }
    ...
    if (currentEditingCard) {
        if (currentEditingCard.isNew) {
            ...
            if (currentBoard) {
                ...
                // 对该板块进行排序
                const sortedCards = sortCards(currentBoard.columns[columnId], columnId);
                currentBoard.columns[columnId] = sortedCards;
...
async function loadAds() {
    try {
        ...
        try {
            ...
            console.error('加载广告数据完全失败:', e2);
        }
    }
}

// 广告数据的保存功能移到setting.js处理
...
async function initAdCarousel() {
    ...
    // 渲染广告（先添加所有广告，再在最后添加第一个广告的副本，用于无缝循环）
    adTrack.innerHTML = '';
    ADS.forEach(ad => {
        const div = document.createElement('div');
...
function updateAdCarousel() {
    ...
    // 应用过渡效果
    adTrack.style.transition = 'transform 0.5s ease';
    adTrack.style.transform = `translateY(-${currentAdIndex * 40}px)`;
    
    // 如果到达了副本位置，立即切换回真正的第一个位置，不带动画
    if (currentAdIndex === ADS.length) {
        setTimeout(() => {
            adTrack.style.transition = 'none';
            currentAdIndex = 0;
            adTrack.style.transform = `translateY(0px)`;
        }, 500); // 等待动画完成
    }
}
